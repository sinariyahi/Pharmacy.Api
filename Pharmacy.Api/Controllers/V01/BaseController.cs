...
namespace Pharmacy.Api.Controllers.V01
{
    ...
    public class BaseController : ControllerBase
    {
        ...
        {
            return new Guid(User.Claims.FirstOrDefault().Value);
        }

        protected IActionResult ShowError(string message)
        {
    ...
    public class AuthBaseController : ControllerBase
    {
        ...
        public AuthBaseController(IAuthenticateService _authenticateService)
        {
        ...
        {
            return await authenticateService.getCurrentUSer(new Guid(User.Claims.FirstOrDefault().Value));
        }
        protected IActionResult ShowError(string message)
        {
            var result = new GSActionResult<object>();
        ...
        protected GSActionResult<object> ShowError(List<string> list)
        {
            var result = new GSActionResult<object>();
            result.Data = list;
            result.Message = string.Join(",", list);
            result.IsSuccess = false;
            return result;
        }
        protected async Task<bool> CheckPermission(Guid moduleId, Guid permissionId)
        {
            var currUser = await authenticateService.getCurrentUSer(new Guid(User.Claims.FirstOrDefault().Value));
            if (currUser.UserTypeId == new Guid("9291CDD8-F956-4431-A707-03A47595BDB9")) /*admin permission is full*/
                return true;
            var allPermissionList = (List<Permission>)currUser.MenuPermissionList;
            var modulePermissionList = allPermissionList.Find(x => x.roleId == moduleId);
            if (modulePermissionList != null)
                return (modulePermissionList.permissionTypeList.ToList().Find(x => x.PermissionId == permissionId) != null) ? true : false;
            else
                return false;
        }

        protected async Task<ActionResult> TableToExcel(DataTable dt, string reportName)
        {
            using (var workbook = new XLWorkbook())
            {
                ...
                {
                    worksheet.Cell(currentRow, c).Value = column.ColumnName;
                    c++;
                }
                foreach (DataRow r in dt.Rows)
                {
                    ...
                    {
                        worksheet.Cell(currentRow, i).Value = (int)r[i - 1];
                    }
                }
                using (var stream = new MemoryStream())
                {
                    ...
                    //content, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",string.Format( "{0}.xlsx",reportName)));
                    content, "application/octet-stream", string.Format("{0}.xlsx", reportName)));
                }
            }
        }
    ...
    public class AuthBaseWithAttachmentController<T> : AuthBaseController
    {
        private readonly IAttachmentService _attachmentService;
        private readonly IGenericAttachmentService _genericAttachmentService;
        private IAttachmentService attachmentService;

        public AuthBaseWithAttachmentController(IAuthenticateService _authenticateService, IAttachmentService attachmentService) : base(_authenticateService)
        {
            this.attachmentService = attachmentService;
        }

        public AuthBaseWithAttachmentController(IAuthenticateService _authenticateService, IAttachmentService attachmentService, IGenericAttachmentService genericAttachmentService) : base(_authenticateService)
        {
            _attachmentService = attachmentService;
            _genericAttachmentService = genericAttachmentService;
        }
        ...
        public async Task<FileResult> GetAttachments([FromQuery] T id, string imageId)
        {
            var attachment = await _attachmentService.GetAttachment(id, imageId, _genericAttachmentService.getAttachmentProcedure());
            if (attachment is null)
                return null;
            if (!string.IsNullOrEmpty(attachment.AttachmentPath))
            {
                string[] arr = attachment.AttachmentPath.Split('.');
                string fileExtention = arr[arr.Length - 1];
                attachment.ContentType = getContentType(fileExtention);
                var stream = System.IO.File.OpenRead(attachment.AttachmentPath);
                return (File(stream, attachment.ContentType, string.Format("{0}.{1}", attachment.AttachmentName, fileExtention)));
            }
            else
                return null;
        ...
        public async Task<GSActionResult<object>> RemoveAttachments(T id, string imageId)
        {
            var r = await _attachmentService.SetAttachment(id, imageId, "", _genericAttachmentService.removeAttachmentProcedure());
            if (r.IsSuccess)
            {
                if (r.Data != null)
                {
                    string removedPath = r.Data.ToString();
                    if (!string.IsNullOrEmpty(removedPath))
                    {
                        string currDir = Directory.GetCurrentDirectory();
                        removedPath = string.Concat(currDir, removedPath);
        ...
        ///بارگذاری فایل
        /// </summary>
        protected async Task<GSActionResult<object>> SetAttachment(dynamic id, string fieldName, string filePath)
        {
            return await _attachmentService.SetAttachment(id, fieldName, filePath, _genericAttachmentService.setAttachmentProcedure());
        }
